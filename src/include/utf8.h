 ...
 * null. */
utf8_nonnull utf8_weak utf8_int8_t *
utf8catcodepoint_safe(utf8_int8_t *str, utf8_int32_t chr, size_t n);
utf8_nonnull utf8_weak utf8_int8_t *
utf8catcodepoint(utf8_int8_t *str, utf8_int32_t chr);
...
int utf8makevalid(utf8_int8_t *str, const utf8_int32_t replacement) {
  ...
  while ('\0' != *read) {
    if (0xf0 == (0xf8 & *read)) {
      ...
      write = utf8catcodepoint(write, codepoint);
...
utf8_int8_t *utf8catcodepoint_safe(utf8_int8_t *str, utf8_int32_t chr, size_t n) {
  if (0 == ((utf8_int32_t)0xffffff80 & chr)) {
    /* 1-byte/7-bit ascii
     * (0b0xxxxxxx) */
    if (n < 1) {
...
utf8_int8_t *utf8catcodepoint(utf8_int8_t *str, utf8_int32_t chr) {
	if (0 == ((utf8_int32_t)0xffffff80 & chr)) {
		/* 1-byte/7-bit ascii
		 * (0b0xxxxxxx) */
		str[0] = (utf8_int8_t)chr;
...
void utf8lwr(utf8_int8_t *utf8_restrict str) {
  ...
  while (cp != 0) {
    ...
    if (lwr_cp != cp) {
      utf8catcodepoint_safe(str, lwr_cp, size);
    }
